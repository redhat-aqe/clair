...
func init() {
	featurefmt.RegisterLister("redhat", "1.0", &lister{})
}
...
func (l lister) ListFeatures(files tarutil.FilesMap) ([]database.LayerFeature, error) {
	namespaces := getPotentialNamespace(files)
	...
	for scanner.Scan() {
		rpmPackage, _ := parseRPMOutput(scanner.Text())
		if rpmPackage != nil {
	...
	totalFeature := len(layerFeatures)
	if len(namespaces) > 0 {
		totalFeature = len(layerFeatures) * len(namespaces)
	}
	layerFeaturesNamespace := make([]database.LayerFeature, 0, totalFeature)
...
func getPotentialNamespace(files tarutil.FilesMap) (namespaces []database.Namespace) {
